...
Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/jspProject",
	...
	out.println("<h2>후기게시판 관리</h2>");
	out.println("<hr><br><br>");
	out.println("<table class='tbl'>");
	out.println("<tr><td colspan=8 width=500 align=right>");
	out.println("<a href='boardAdd.html'>게시물관리</a></td></tr>");
	
	out.println("<tr><td>ID</td><td>장소</td><td>테마</td><td>제목</td><td>사진</td><td>날짜</td><td>삭제</td>");
	...
	while(rs.next()) {
		...
		out.println("<td><img src='imageView2.jsp?title=" + rs.getString("title") + "' width=100></td>");
		out.println("<td>" + rs.getString("date") + "</td>");	
		out.println("<td><a href ='boardDelete.jsp?title="+rs.getString("title")+"'>[Delete]</a></td>");
		out.println("</tr>");
	}
	out.println("</table>");
	out.println("</center>");
