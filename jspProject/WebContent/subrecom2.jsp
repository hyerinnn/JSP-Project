...
Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/jspProject",
	...
	ResultSet rs = st.executeQuery("SELECT * FROM recom where title= '"+request.getParameter("title")+"'");
	
	out.println("<center><br>");
	out.println("<h2>추천받기</h2>");
	out.println("<hr><br><br>");
out.println("<table class='tbl' boarder=1 cellspacing='1' cellpadding='5'>");
	
	
	while(rs.next()) {
		out.println("<tr>");
		String s = rs.getString("uid");
		out.println("<tr><th>*ID : </td><td>" + s + "</td></tr>");
		out.println("<tr><th>*제목 : </td><td>" + rs.getString("title") + "</th></tr>");
		out.println("<tr><th>*지역 : </td><td>" + rs.getString("place") + "</th></tr>");
		out.println("<tr><th>*계절 : </td><td>" + rs.getString("season") + "</th></tr>");
		out.println("<tr><th>*테마 : </td><td>" + rs.getString("tema") + "</th></tr>");
		out.println("<tr><th>*이미지 : </td><td><img src='imageView3.jsp?title=" + rs.getString("title") + "' width=300></th></tr>");
